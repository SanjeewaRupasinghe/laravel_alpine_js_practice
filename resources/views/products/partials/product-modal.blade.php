<div x-show="isModalOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    ...
    <div class="bg-white p-6 rounded-lg shadow-lg w-[500px]">
        ...
        <!-- form -->
        <form :action="mode === 'edit' ? `/products/${form.id}` : '/products'" method="POST"
            ...
            <div class="mb-4">
                ...
                <input x-model="form.name" value="{{ old('name') }}" :disabled="isView" type="text"
                    id="name" name="name" class="mt-1 p-2 border rounded w-full">
                @error('name')
            ...
            <div class="mb-4 grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input x-model="form.price" value="{{ old('price') }}" :disabled="isView" type="number"
                        id="price" name="price" class="mt-1 p-2 border rounded w-full">
                    @error('price')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select x-model="form.status" :disabled="isView" id="status" name="status"
            ...
            <div class="mb-4">
                ...
                <textarea x-model="form.description" value="{{ old('description') }}" :disabled="isView" id="description"
                    name="description" class="mt-1 p-2 border rounded w-full"></textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <div x-show="!isView" @click="$refs.images.click()" @dragover.prevent @drop.prevent="handleDrop($event)"
                    ...
                    <input @change="handleImage($event)" x-ref="images" :disabled="isView" type="file"
                        class="hidden" id="images" name="images[]" multiple accept="image/*">
            ...
            <div class="flex justify-end w-full">
                ...
                <button x-show="!isView" type="submit"
